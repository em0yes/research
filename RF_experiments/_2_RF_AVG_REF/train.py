...
def train_random_forest():
    ...
    rfe = RFE(estimator=best_rf, n_features_to_select=15)
    X_train_rfe = rfe.fit_transform(X_train, y_train_encoded)
    X_val_rfe = rfe.transform(X_val)

    # 최적화된 Random Forest 모델 학습
    best_rf.fit(X_train_rfe, y_train_encoded)

    # 검증 데이터로 예측
    y_pred_encoded = best_rf.predict(X_val_rfe)
    ...
    # 학습된 모델과 RFE 선택기, 레이블 인코더 저장
    joblib.dump(best_rf, '_2_RF_AVG_REF(O)_shuffle(O)_model.pkl')
    joblib.dump(rfe, '_2_RF_AVG_REF(O)_shuffle(O)_selector.pkl')
    joblib.dump(le, '_2_RF_AVG_REF(O)_shuffle(O)_label_encoder.pkl')

    print("Optimized model, RFE selector, and label encoder saved.")
