...
class ZoneClassifierWithSlidingWindow:
    def __init__(self, window_size_rows=5, step_size_rows=5, n_estimators=100):
        # GradientBoostingClassifier 초기화 (기본 설정 사용)
    ...
    def preprocess_data(self, data):
        ...
        while start_index + self.window_size <= len(data):
            ...
            if not window_data.empty and window_data['Zone'].nunique() == 1:
                # 숫자 열에 대해서만 NaN을 0으로 채움
                window_data_no_zero = window_data.drop(columns=['TimeStamp', 'Zone']).apply(pd.to_numeric, errors='coerce').fillna(0)
                grouped_data.append(window_data_no_zero.mean(skipna=True).tolist())
    ...
    def train_model(self):
        """
        학습 데이터를 통해 GradientBoosting 모델을 학습합니다.
        """
...
def main(train_file, test_file, model_name, report_file):
    # 슬라이딩 윈도우 기반 분류기 인스턴스 생성 (GradientBoosting 사용)
    classifier = ZoneClassifierWithSlidingWindow(window_size_rows=5, step_size_rows=1, n_estimators=100)
...
model_name = 'gb'  # 모델 기본 이름 (GradientBoosting)
