...
class ZoneClassifierWithSlidingWindow:
    def __init__(self, window_size_rows=5, step_size_rows=5, kernel='linear'):
        # SVC 초기화 (기본 설정 사용, linear kernel 사용)
...
def main(train_file, test_file, model_name, report_file):
    # 슬라이딩 윈도우 기반 분류기 인스턴스 생성 (SVM 사용)
    classifier = ZoneClassifierWithSlidingWindow(window_size_rows=5, step_size_rows=1, kernel='linear')  # linear 커널 사용
