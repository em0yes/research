...
from sklearn.impute import SimpleImputer  # NaN 값을 처리하기 위한 임퓨터
import pickle

class ZoneClassifierWithSlidingWindow:
    def __init__(self, window_size_rows=5, step_size_rows=5, n_neighbors=5):
        # KNeighborsClassifier 초기화 (기본 설정 사용)
        ...
        self.imputer = SimpleImputer(strategy='mean')  # NaN 값을 평균으로 대체
    ...
    def preprocess_data(self, data):
        ...
        # NaN 값을 평균으로 대체
        grouped_data = self.imputer.fit_transform(pd.DataFrame(grouped_data))

        return pd.DataFrame(grouped_data), grouped_labels

    def train_model(self):
        """
        학습 데이터를 통해 KNN 모델을 학습합니다.
        """
...
def main(train_file, test_file, model_name, report_file):
    # 슬라이딩 윈도우 기반 분류기 인스턴스 생성 (KNN 사용)
    classifier = ZoneClassifierWithSlidingWindow(window_size_rows=5, step_size_rows=1, n_neighbors=5)
