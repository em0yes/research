...
class ZoneClassifierWithSlidingWindow:
    def __init__(self, window_size_rows=5, step_size_rows=5, n_estimators=100, learning_rate=0.1):
        # XGBClassifier 초기화
        self.model = XGBClassifier(n_estimators=n_estimators, learning_rate=learning_rate, random_state=42)
    ...
    def preprocess_data(self, data):
        ...
        while start_index + self.window_size <= len(data):
            ...
            if not window_data.empty and window_data['Zone'].nunique() == 1:
                grouped_data.append(window_data.drop(columns=['TimeStamp', 'Zone']).mean().tolist())
...
def main(train_file, test_file, model_name, report_file):
    # 슬라이딩 윈도우 기반 분류기 인스턴스 생성 (XGBoost 사용)
    classifier = ZoneClassifierWithSlidingWindow(window_size_rows=5, step_size_rows=1, n_estimators=100, learning_rate=0.1)
...
model_name = 'xgb'  # 모델 기본 이름 (XGBoost)
