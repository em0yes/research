...
class ZoneClassifierWithSlidingWindow:
    ...
    def preprocess_data(self, data):
        """
        데이터를 슬라이딩 윈도우 방식으로 그룹화하고, NaN 값을 처리합니다.
        """
        ...
        while start_index + self.window_size <= len(data):
            ...
            if not window_data.empty and window_data['Zone'].nunique() == 1:
                # 유효한 윈도우만 데이터로 처리, NaN 값은 0으로 대체
                window_data_no_zero = window_data.drop(columns=['TimeStamp', 'Zone']).mean(skipna=True)
                window_data_no_zero = window_data_no_zero.fillna(0).infer_objects()  # fillna() 후 데이터 타입 처리
                grouped_data.append(window_data_no_zero.tolist())
    ...
    def train_model(self):
        """
        학습 데이터를 통해 LogisticRegression 모델을 학습합니다.
        """
...
def main(train_file, test_file, model_name, report_file):
    # 슬라이딩 윈도우 기반 분류기 인스턴스 생성 (LogisticRegression 사용)
...
model_name = 'lr'  # 모델 기본 이름 (LogisticRegression)
